...
def sync_table_until_empty(datacore, snowflake, sync_log_id, table_config_row):
    ...
    log(f"{schema_name}.{table_name}: draining table until empty...")
    ...
    while True:
        ...
        try:
            log(f"{schema_name}.{table_name}: fetching batch from Snowflake...")

            sf_columns, sf_rows = snowflake.run_when_available(
                fetch_snowflake_batch,
                table_name=table_name,
                pk_column=pk_column,
            ...
            log(f"{schema_name}.{table_name}: fetched {len(sf_rows):,} rows from Snowflake.")
            ...
            valid_rows, null_pk_rows = split_rows_by_required_pk(
                columns=sf_columns,
                rows=sf_rows,
                pk_column=pk_column,
            )
            ...
            log(f"{schema_name}.{table_name}: checking payload columns exist in target/staging...")
            ...
            target_columns_added = datacore.run_when_available(
                ensure_columns_exist_from_payload,
                schema_name=schema_name,
                table_name=table_name,
            ...
            # next batch.
            log(f"{schema_name}.{table_name}: clearing staging table...")
            ...
            log(f"{schema_name}.{table_name}: inserting {len(sf_rows):,} rows into staging...")

            datacore.run_when_available(
                ingest_staging_datacore,
                schema_name=schema_name,
                table_name=table_name,
                rows=sf_rows,
                columns=sf_columns
            ...
            log(f"{schema_name}.{table_name}: merging staging into target...")
            ...
            if schema_name == "reapit" and table_name == "jnl":
                log(f"{schema_name}.{table_name}: upserting certificate events from staging...")

                event_result = datacore.run_when_available(
                    upsert_certificate_events_from_staging,
                    rps_instance="core",
                    sync_log_item_id=sync_log_item_id,
            ...
            last_synced, last_pk = get_batch_watermark(
                columns=sf_columns,
                rows=sf_rows,
                pk_column=pk_column
            )
            ...
            log(f"{schema_name}.{table_name}: sync log item updated successfully.")

            datacore.run_when_available(
                update_table_config_watermark,
                schema_name=schema_name,
                table_name=table_name,
                last_synced=last_synced,
                last_pk=last_pk
            ...
            log(f"{schema_name}.{table_name}: table_config watermark updated.")
            ...
            log(f"batch {batch_number} failed: {type(e).__name__}: {repr(e)}")
